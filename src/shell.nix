...
let
  ...
  pushUpDevshellOptions = config:
    let
      movedOpts = l.flip l.filterAttrs config (name: _:
        l.warnIf
  ...
  baseConfig = {
    ...
    env = [
      { name = "LD_LIBRARY_PATH"; eval = "$DEVSHELL_DIR/lib:${l.makeLibraryPath common.runtimeLibs}"; }
